/*
 Este programa esta hecho a partir del ejemplo de raytracer basico de
 scratchapixel (migrado a lenguaje C).
*/
/*
...
//  unsigned width = 640, height = 480;
  ...
  //MY CODE 
  //#pragma omp parallel for private(x, xx, yy) shared(pixel)
  
 #pragma omp parallel for schedule(dynamic,1) private(x, xx, yy, aux, raydir) firstprivate(pixel) 
 
  for (y = 0; y < height; ++y) {
	  pixel=&image[y*width];
    for (x = 0; x < width; ++x, ++pixel) {xx = (2 * ((x + 0.5) * invWidth) - 1) * angle * aspectratio;
